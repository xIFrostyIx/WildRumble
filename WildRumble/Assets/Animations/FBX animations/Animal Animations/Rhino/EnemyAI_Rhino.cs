...
public class EnemyAI_Rhino : MonoBehaviour
{
    ...
    IEnumerator Start()
    {
        ...
        while (true)
        {
            ...
            myAnimator.SetInteger("rhinoIdle_Index", Random.Range(0, 3));
            myAnimator.SetTrigger("rhinoIdle");
        }
    }
    ...
    void Update()
    {
        ...
        if (distanceToPlayer < chaseRange)
        {
            ...
            if (distanceToPlayer > stoppingDistance)
            {
                ...
                myAnimator.SetBool("rhinoWalk", true);
                myAnimator.SetBool("rhinoAttack", false);
                myAnimator.SetBool("rhinoIdle_Bool", false);
            }
            else
            {
                navMeshAgent.SetDestination(transform.position); // Stop moving

                myAnimator.SetBool("rhinoAttack", true);
                myAnimator.SetBool("rhinoWalk", false);
                myAnimator.SetBool("rhinoIdle", false);
                myAnimator.SetBool("rhinoIdle_Bool", false);
            }
        }
        else
