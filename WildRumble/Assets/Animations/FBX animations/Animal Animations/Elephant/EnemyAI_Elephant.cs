...
public class EnemyAI_Elephant : MonoBehaviour
{
    ...
    IEnumerator Start()
    {
        ...
        while (true)
        {
            ...
            myAnimator.SetInteger("elephantIdle_Index", Random.Range(0, 3));
            myAnimator.SetTrigger("elephantIdle");
        }
    }
    ...
    void Update()
    {
        ...
        if (distanceToPlayer < chaseRange)
        {
            ...
            if (distanceToPlayer > stoppingDistance)
            {
                ...
                myAnimator.SetBool("elephantWalk", true);
                myAnimator.SetBool("elephantAttack", false);
                myAnimator.SetBool("elephantIdle_Bool", false);
            }
            else
            {
                navMeshAgent.SetDestination(transform.position); // Stop moving

                myAnimator.SetBool("elephantAttack", true);
                myAnimator.SetBool("elephantWalk", false);
                myAnimator.SetBool("elephantIdle", false);
                myAnimator.SetBool("elephantIdle_Bool", false);
            }
        }
        else
