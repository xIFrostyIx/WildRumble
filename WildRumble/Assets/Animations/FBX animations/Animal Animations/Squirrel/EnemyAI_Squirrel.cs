...
public class EnemyAI_Squirrel : MonoBehaviour
{
    ...
    IEnumerator Start()
    {
        ...
        while (true)
        {
            ...
            myAnimator.SetInteger("squirrelIdle_Index", Random.Range(0, 3));
            myAnimator.SetTrigger("squirrelIdle");
        }
    }
    ...
    void Update()
    {
        ...
        if (distanceToPlayer < chaseRange)
        {
            ...
            if (distanceToPlayer > stoppingDistance)
            {
                ...
                myAnimator.SetBool("squirrelWalk", true);
                myAnimator.SetBool("squirrelAttack", false);
                myAnimator.SetBool("squirrelIdle_Bool", false);
            }
            else
            {
                navMeshAgent.SetDestination(transform.position); // Stop moving

                myAnimator.SetBool("squirrelAttack", true);
                myAnimator.SetBool("squirrelWalk", false);
                myAnimator.SetBool("squirrelIdle", false);
                myAnimator.SetBool("squirrelIdle_Bool", false);
            }
        }
        else
