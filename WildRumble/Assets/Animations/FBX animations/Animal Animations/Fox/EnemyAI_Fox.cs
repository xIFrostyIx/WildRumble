...
public class EnemyAI_Fox : MonoBehaviour
{
    ...
    IEnumerator Start()
    {
        ...
        while (true)
        {
            ...
            myAnimator.SetInteger("foxIdle_Index", Random.Range(0, 3));
            myAnimator.SetTrigger("foxIdle");
        }
    }
    ...
    void Update()
    {
        ...
        if (distanceToPlayer < chaseRange)
        {
            ...
            if (distanceToPlayer > stoppingDistance)
            {
                ...
                myAnimator.SetBool("foxWalk", true);
                myAnimator.SetBool("foxAttack", false);
                myAnimator.SetBool("foxIdle_Bool", false);
            }
            else
            {
                navMeshAgent.SetDestination(transform.position); // Stop moving

                myAnimator.SetBool("foxAttack", true);
                myAnimator.SetBool("foxWalk", false);
                myAnimator.SetBool("foxIdle_Bool", false);
            }
        }
        else
