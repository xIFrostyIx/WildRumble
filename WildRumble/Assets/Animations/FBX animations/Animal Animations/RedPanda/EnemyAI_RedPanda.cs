...
public class EnemyAI_RedPanda : MonoBehaviour
{
    ...
    IEnumerator Start()
    {
        ...
        while (true)
        {
            ...
            myAnimator.SetInteger("redPandaIdle_Index", Random.Range(0, 3));
            myAnimator.SetTrigger("redPandaIdle");
        }
    }
    ...
    void Update()
    {
        ...
        if (distanceToPlayer < chaseRange)
        {
            ...
            if (distanceToPlayer > stoppingDistance)
            {
                ...
                myAnimator.SetBool("redPandaWalk", true);
                myAnimator.SetBool("redPandaAttack", false);
                myAnimator.SetBool("redPandaIdle_Bool", false);
            }
            else
            {
                navMeshAgent.SetDestination(transform.position); // Stop moving

                myAnimator.SetBool("redPandaAttack", true);
                myAnimator.SetBool("redPandaWalk", false);
                myAnimator.SetBool("redPandaIdle", false);
                myAnimator.SetBool("redPandaIdle_Bool", false);
            }
        }
        else
