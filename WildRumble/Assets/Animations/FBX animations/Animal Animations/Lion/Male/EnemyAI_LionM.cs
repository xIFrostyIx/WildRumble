...
public class EnemyAI_LionM : MonoBehaviour
{
    ...
    IEnumerator Start()
    {
        ...
        while (true)
        {
            ...
            myAnimator.SetInteger("lionMIdle_Index", Random.Range(0, 3));
            myAnimator.SetTrigger("lionMIdle");
        }
    }
    ...
    void Update()
    {
        ...
        if (distanceToPlayer < chaseRange)
        {
            ...
            if (distanceToPlayer > stoppingDistance)
            {
                ...
                myAnimator.SetBool("lionMWalk", true);
                myAnimator.SetBool("lionMAttack", false);
                myAnimator.SetBool("lionMIdle_Bool", false);
            }
            else
            {
                navMeshAgent.SetDestination(transform.position); // Stop moving

                myAnimator.SetBool("lionMAttack", true);
                myAnimator.SetBool("lionMWalk", false);
                myAnimator.SetBool("lionMIdle", false);
                myAnimator.SetBool("lionMIdle_Bool", false);
            }
        }
        else
